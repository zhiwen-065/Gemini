  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      const isRetryable =
        err?.status === 500 ||
        err?.status === 429 ||
        String(err?.message || '').includes('Rpc failed');
...
function getStoredApiKey(): string {
  return localStorage.getItem('ALGO_GEMINI_API_KEY') || '';
}

function setStoredApiKey(key: string) {
  localStorage.setItem('ALGO_GEMINI_API_KEY', key);
}
  ...
  "videoList": [
    {
      ...
      const avatarPromise = callWithRetry(() =>
        ai.models.generateContent({
    ...
    <div className="h-screen bg-black text-zinc-100 flex flex-col font-sans overflow-hidden select-none">
      ...
      {loading && (
        <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-black">
          ...
          <p className="text-[10px] font-black tracking-[0.5em] text-red-500 uppercase animate-pulse">
      ...
      {step === 'welcome' && (
        <div className="flex-1 flex flex-col items-center justify-center p-8 space-y-12 animate-in fade-in duration-1000">
          <div className="text-center space-y-2">
            <h1 className="text-8xl font-black italic tracking-tighter bg-gradient-to-b from-white to-zinc-800 bg-clip-text text-transparent">
              ALGO
            </h1>
          ...
          <div className="flex flex-col gap-3 w-full max-w-xs">
            <button
              ...
              className="w-full py-6 bg-white text-black font-black rounded-full text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-2xl"
            >
              启动模拟器
            </button>
            ...
            <button
              ...
              className="w-full py-4 bg-white/10 border border-white/10 text-white/80 font-black rounded-full text-[10px] uppercase tracking-widest hover:bg-white/15 transition-all"
            >
              设置 Gemini API Key
            </button>
          </div>
            ...
            <div className="space-y-8">
                ...
                {['男', '女'].map(g => (
                  <button
                    ...
                    className={`flex-1 py-4 rounded-2xl border-2 font-black transition-all ${
                      formData.gender === g ? 'bg-red-600 border-red-600' : 'border-zinc-800 text-zinc-600'
                    }`}
                  >
                    {g}
                  </button>
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-[10px] font-black uppercase text-zinc-500">
                  年龄: <span className="text-red-500 text-lg italic">{formData.age}</span>
                </div>
                <input
                  type="range"
                  min="15"
              ...
              <input
                ...
                className="w-full bg-zinc-900 border-2 border-zinc-800 rounded-2xl px-6 py-4 text-sm outline-none focus:border-red-600 transition-all"
              />

              <button
                onClick={startGeneration}
                className="w-full py-5 bg-red-600 rounded-full font-black text-xs uppercase tracking-widest shadow-xl"
              >
                生成首个画像
              </button>
            </div>
        ...
        <div className="flex-1 flex flex-col h-full relative">
          ...
          <div className="px-4 py-4 border-b border-white/5 bg-black/60 backdrop-blur-3xl z-50">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-3 items-center">
                ...
                <button
                  onClick={() => setShowKeyModal(true)}
                  className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-[10px] font-black"
                  title="设置 API Key"
                >
                  KEY
                </button>
                <button onClick={() => setStep('welcome')} className="text-[9px] opacity-30 hover:opacity-100 uppercase font-black">
                  Quit
                </button>
              </div>
            </div>
            ...
            <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
              {personas.map(p => (
                <div
                  ...
                  className={`relative flex-shrink-0 w-14 h-20 rounded-2xl overflow-hidden cursor-pointer border-2 transition-all ${
                    activePersonaId === p.id ? 'border-red-600 scale-105' : 'border-transparent opacity-40'
                  }`}
                >
                  {p.isLoading ? (
                    ...
                    <>
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); setZoomCardId(p.id); }}
                        className="absolute bottom-1 right-1 w-4 h-4 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center text-[8px]"
                        title="查看档案"
                      >
                        👁️
                      </button>
          ...
          <div ref={feedRef} className="flex-1 overflow-y-auto snap-y snap-mandatory no-scrollbar bg-black">
            {activePersona && !activePersona.isLoading ? (
              activePersona.videos.map((v, i) => (
                <div key={v.id} className="h-full w-full snap-start relative flex flex-col justify-end p-6 pb-12 overflow-hidden">
                  <div className="absolute inset-0">
                    {v.imageUrl
                      ? <img src={v.imageUrl} className="w-full h-full object-cover opacity-70" />
                  ...
                  <div className="absolute right-4 bottom-12 flex flex-col gap-6 items-center z-20">
                    ...
                    <button
                      ...
                      className={`w-12 h-12 rounded-full flex items-center justify-center text-xl backdrop-blur-xl border-2 transition-all ${
                        revealedLogicId === v.id ? 'bg-red-600 border-red-400 scale-110' : 'bg-black/40 border-white/5'
                      }`}
                      title="显示推送逻辑"
                    >
                      🧠
              ...
              <div className="h-full flex items-center justify-center text-zinc-800 text-[10px] font-black uppercase tracking-[0.5em]">
                载入神经网络流...
              </div>
            )}
          </div>
            ...
            <div className="space-y-6">
              <div className="flex gap-2">
                {['男', '女'].map(g => (
                  <button
                    key={g}
                    onClick={() => setFormData(p => ({ ...p, gender: g }))}
                    className={`flex-1 py-3 rounded-xl border-2 font-black transition-all text-xs ${
                      formData.gender === g ? 'bg-red-600 border-red-600' : 'border-zinc-800 text-zinc-600'
                    }`}
                  >
                    {g}
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-[9px] font-black uppercase text-zinc-500">
                  年龄: <span className="text-red-500">{formData.age}</span>
                </div>
                <input
                  type="range"
                  min="15"
              ...
              <input
                ...
                value={formData.interests}
                onChange={e => setFormData(p => ({ ...p, interests: e.target.value }))}
                className="w-full bg-zinc-900 border-2 border-zinc-800 rounded-xl px-4 py-3 text-xs outline-none"
              />

              <button
                onClick={startGeneration}
                className="w-full py-4 bg-red-600 rounded-full font-black text-xs uppercase tracking-widest"
              >
                确认分发
              </button>
            </div>
            ...
            <div className="w-full max-w-xs glass rounded-[40px] p-8 space-y-8 animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  ...
                  <div className="text-[11px] font-bold text-red-500 bg-red-500/5 p-3 rounded-2xl border border-red-500/10 italic leading-relaxed">
                    {p.lifeStage}
                  </div>
                </div>
              ...
              <button
                onClick={() => setZoomCardId(null)}
                className="w-full py-4 bg-zinc-100 text-black font-black rounded-full text-[10px] uppercase"
              >
                关闭档案
              </button>
            </div>
      ...
      {/* API Key Modal */}
      {showKeyModal && (
        <div className="fixed inset-0 z-[210] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm" onClick={() => setShowKeyModal(false)}>
          <div className="w-full max-w-sm glass p-8 rounded-[28px] space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-black italic">设置 Gemini API Key</h3>
              <button className="text-white/50 hover:text-white" onClick={() => setShowKeyModal(false)}>✕</button>
            </div>

            <p className="text-[11px] text-white/50 leading-relaxed">
              该 Key 会存到本机 localStorage（只在你的浏览器里），用于调用 Gemini 文本与图片生成。
              不建议在公共电脑输入。
            </p>
            ...
            <input
              ...
              className="w-full bg-zinc-900 border-2 border-zinc-800 rounded-xl px-4 py-3 text-xs outline-none focus:border-red-600"
            />
            ...
            <div className="flex gap-2">
              <button
                onClick={() => {
                  ...
                  alert('已保存 API Key（仅本机浏览器）。');
                }}
                className="flex-1 py-3 bg-red-600 rounded-full font-black text-xs uppercase tracking-widest"
              >
                保存
              </button>
              <button
                ...
                className="flex-1 py-3 bg-white/10 border border-white/10 rounded-full font-black text-xs uppercase tracking-widest"
              >
                清除
              </button>
            </div>
